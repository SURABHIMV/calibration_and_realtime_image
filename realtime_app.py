...
def preprocess_image():
    if request.method == 'POST':
        ...
        #Retriving the uploaded image name
        ...
        #Storing the binary_mask image in upload folder
        ...
        #Storing the original image inside static folder
        ...
        # Convert the contours to nested lists so to obtain the number of contours
...
def mask_image():
    if request.method == 'POST':
        # Retrieving the number of contours stored in session
        num_contours = session.get('num_contours')
        ln = []
        wd=[]
        #Based on the number of contours a for loop is formed. so that at each i actual length and width is added as input and stored in lists
        ...
        # Retrieve the binary mask image path from the session
        ...
        for i, contour in enumerate(contours):
            ...
            #width and length are the pixel values
            w=width
            
            h=height
            #Obtaining the mf for length m
            m=ln[i]/h
            #obtaining the mf for width w
            f=wd[i]/w
            MF_l.append(m)
            MF_w.append(f)
        ...
        #data.text stores the dictionary as in string form
